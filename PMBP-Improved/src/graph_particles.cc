...
void GraphParticles::InitialiseImages(Image* one, Image* two,float***data,int channel)
{
  images[kOne] = one;
  images[kTwo] = two;
  w[kOne] = one->width;
  ...
  image_operator = new ImageOperator(data,channel,images, gradients, filtered, w, h, parameters, displacement_function);
}
