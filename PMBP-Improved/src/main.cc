...
bool process_args(int argc, char* argv[], Application &application, Parameters& parameters)
{
  ...
  while (1)
  {
    ...
    else if (std::string(argv[pos]) == "-Data1")                    { sprintf(parameters.data_one ,"%s",argv[++pos]); pos++; }
    else if (std::string(argv[pos]) == "-Data2")                    { sprintf(parameters.data_two ,"%s",argv[++pos]); pos++; }
...
void LoadData(float**&data,const char* filename)
{
	int width;
	int height; 
...
void run(const Parameters& parameters, Application application){
  ...
  float***data;
  data=new float**[2];
 LoadData(data[0],parameters.data_one);
LoadData(data[1],parameters.data_two);

  graph->InitialiseImages(one, two,data,channel);
  graph->Solve();
